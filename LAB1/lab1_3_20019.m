...
x=sin(2*pi*100*t)+ 0.3*sin(2*pi*150*(t-2))+ sin(2*pi*200*t); % ημιτονικό σήμα συχνότητας 100 Hz
 % συνιστώσα 150 Hz
 % συνιστώσα 200 Hz
...
title('Time domain plot of x (ANNA KOUTSONI)') % τίτλος γραφικής παράστασης
...
plot((f(1:N)),abs(X(1:N))) % γραφική παράσταση των θετικών συχνοτή-των
title('Frequency domain plot of x (ANNA KOUTSONI)') % τίτλος γραφικής παράστασης
...
plot(f,abs(X));title('Two sided spectrum of x (ANNA KOUTSONI)'); xlabel('f (Hz)');
ylabel('Amplitude')
pause % αναμονή, πιέστε ένα πλήκτρο για να συνεχίσετε
% Υπολογίστε την ισχύ
...
title('{\bf Periodogram} (ANNA KOUTSONI)') % τίτλος διαγράμματος με παχιά γράμματα
pause
disp('Part2')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
axis([0 0.3 -2 2]) % εμφάνιση του σήματος από 0 έως 0.3 sec και
 % κλίμακα από -2 έως 2
title('Time domain plot of n (ANNA KOUTSONI)') % τίτλος γραφικής παράστασης
xlabel('t (sec)') % λεζάντα στον άξονα x
ylabel('Noise') % λεζάντα στον άξονα y
pause % αναμονή, πιέστε ένα πλήκτρο για να συνεχίσετε

Nf=fft(n,N);
powern=Nf.*conj(Nf)/N/L; % υπολογισμός πυκνότητας φάσματος ισχύος
figure(6) % άνοιγμα παραθύρου για γραφική παράσταση
f1=(0:N-1)*Fo;
plot(f1,powern) % ισχύς ανά συνιστώσα συχνότητα
xlabel('Frequency (Hz)') % λεζάντα στον άξονα x
ylabel('Power') % λεζάντα στον άξονα y
title('{\bf Periodogram} (ANNA KOUTSONI)') % τίτλος διαγράμματος με παχιά γράμματα
pause
...
axis([0 0.2 -2 2]) % εμφάνιση του σήματος από 0 έως 0.2 sec και
 % κλίμακα από -2 έως 2
title('Time domain plot of s (ANNA KOUTSONI)') % τίτλος γραφικής παράστασης
xlabel('t (sec)') % λεζάντα στον άξονα x
ylabel('Amplitude') % λεζάντα στον άξονα y
pause % αναμονή, πιέστε ένα πλήκτρο για να συνεχίσετε
...
S=fftshift(S); % ολίσθηση της μηδενικής συχνότητας στο κέντρο
 % του φάσματος
 % (ακολουθούν πολλές εντολές σε μια γραμμή)
plot(f,abs(S));title('Two sided spectrum of s (ANNA KOUTSONI)'); xlabel('f (Hz)');
ylabel('Amplitude');
pause %
...
plot(t,sk) % γραφική παράσταση του σήματος
axis([0 0.2 -2 2]) % εμφάνιση του σήματος από 0 έως 0.2 sec και
 % κλίμακα από -2 έως 2
title('Time domain plot of sk (ANNA KOUTSONI)') % τίτλος γραφικής παράστασης
xlabel('t (sec)') % λεζάντα στον άξονα x
ylabel('Amplitude') % λεζάντα στον άξονα y
pause % αναμονή, πιέστε ένα πλήκτρο για να συνεχίσετε
SK=fft(sk,N);
SK=fftshift(SK);
figure(10) % άνοιγμα παραθύρου για γραφική παράστα-ση
plot((f(1:N)),abs(SK(1:N))) % γραφική παράσταση των θετικών συχνοτή-των
title('Frequency domain plot of SK (ANNA KOUTSONI)') % τίτλος γραφικής παράστασης
xlabel('f (Hz)') % λεζάντα στον άξονα x
ylabel('Amplitude') % λεζάντα στον άξονα y
pause % αναμονή για να δείτε το σχήμα
 % πιέστε ένα πλήκτρο για να συνεχίσετε
